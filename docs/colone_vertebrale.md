...
| **Physique** | Énergie | $E$ | $[0, \infty[$ | **Ressource vitale.** Consommée ($K_{thermo}$), partagée (Osmose), divisée (Mitose). Mort si $E \le 0$. |
...
* C++ Pur : Pour la performance brute, la gestion fine de la mémoire et l'optimisation nécessaires au traitement de grilles 3D massives et de calculs intensifs.
* raylib & ImGui : Pour l'interface graphique, permettant une visualisation 3D en temps réel et un contrôle interactif des paramètres de la simulation. L'observation directe est cruciale pour l'analyse.
* OpenMP : Pour le parallélisme, afin d'exploiter la puissance des processeurs multi-cœurs et d'accélérer les phases de calcul les plus lourdes, sans sacrifier le déterminisme.

L'univers du SED est une Grille de Voxels 3D, un espace discret où chaque position est unique. Le temps, lui aussi, est discret, s'écoulant par cycles de simulation. La garantie du déterminisme absolu, même en environnement parallèle, repose sur un cycle d'exécution en trois phases, souvent appelé "Architecture Tri-Phase" :

1. Phase 1 : Décision (Parallèle, Lecture Seule) À l'instant t, une copie en lecture seule de l'état du monde est créée. Toutes les cellules, sans exception, évaluent leur environnement local sur la base de cette copie immuable. Elles calculent leurs actions désirées (mouvement, division, échange) et les enregistrent dans des listes d' "intentions" temporaires. À ce stade, aucune modification n'est apportée à l'état du monde, ce qui permet de paralléliser massivement cette phase sans risque de conflit.
2. Phase 2 : Résolution (Séquentielle) Les listes d'intentions sont traitées. C'est ici que les conflits sont résolus selon des règles déterministes strictes. Par exemple, si plusieurs cellules visent la même case vide, un critère non ambigu (comme la "dette de besoin" la plus élevée, D) désigne un unique vainqueur. Cette phase est séquentielle pour garantir que la résolution des conflits est univoque et indépendante de l'ordre de traitement des threads.
3. Phase 3 : Application et Mise à Jour (Écriture) Une fois tous les conflits résolus, les actions validées sont appliquées à la grille principale pour construire l'état du monde à l'instant t+1. Parallèlement, les changements d'état passifs, qui ne dépendent que de l'état interne de la cellule (comme la consommation d'énergie, le vieillissement ou l'accumulation de stress), sont également appliqués.

Ce cycle rigoureux constitue l'horlogerie de l'univers. Il garantit que le passage de t à t+1 est une fonction mathématique pure. Les lois qui régissent les calculs effectués durant ce cycle forment la physique de ce monde.
