...
| **Physique** | Énergie | $E$ | $[0, \infty[$ | **Ressource vitale.** Consommée ($k_{\text{thermo}}$), partagée (Osmose), divisée (Mitose). Mort si $E \le 0$. |
| | Dette | $D$ | $[0, \infty[$ | **Entropie Interne ("Faim").** Augmente constamment ($\Delta D_{\text{tick}}$). Sert de **priorité motrice** dans l'arbitrage des conflits. |
| | Stress | $C$ | $[0, 1]$ | **Charge Thermique/Pression.** Représente le dommage structurel/émotionnel. Mortelle si $C > S_c$. |
| | Seuil Critique | $S_c$ | $[0, 1]$ | **Résistance Structurelle.** Tolérance maximale au stress $C$ avant effondrement. |
| | Génétique | $R$ | $[0, 1]$ | **Fréquence de Résonance.** Fixe la compatibilité pour l'Osmose (échange d'énergie). |
...
| | Synapses | $W$ | $[0, 1]^8$ | **Poids de Connexion.** Force des liens virtuels avec les 8 voisins. Modifiable par l'apprentissage (Hebb). |
...
| **Spatial** | Gradient | $G$ | $[0, 1]$ | **Information de Position.** $G \approx 1$ au centre du cluster. Utilisé pour la différenciation. |
| | Coût Pending | $E_{\text{cost}}$ | $[0, \infty[$ | Accumulateur temporaire du coût énergétique des *spikes* neuronaux. |
...
| :--- | :--- | :---: | :--- | :--- |
| **Thermodynamique** | Coût Métabolique | $k_{\text{thermo}}$ | $0.001$ | Perte d'énergie fixe par cycle pour toute cellule vivante. |
| | Taux Entropie | $\Delta D_{\text{tick}}$ | $0.002$ | Vitesse d'augmentation de la Dette $D$ (Faim). |
| | Taux Ennui | $\Delta L_{\text{tick}}$ | $0.001$ | Vitesse d'augmentation de l'Ennui $L$. |
| | Facteur Oubli | $m_{\text{decay}}$ | $0.99$ | Facteur de décroissance exponentielle de la Mémoire $M$. |
| | Coût Mouvement | $C_{\text{mvt}}$ | $0.01$ | Énergie soustraite si le mouvement est validé. |
| **Reproduction/Échange** | Seuil Division | $S_{\text{div}}$ | $1.8$ | $E$ minimale requise pour la mitose. |
| | Facteur Échange | $F_{\text{ech}}$ | $0.1$ | Pourcentage de la différence d'énergie échangée (Osmose). |
| | Max Flux | $\Phi_{\max}$ | $0.05$ | Limite absolue de $\Delta E$ échangée par lien, pour la stabilité numérique. |
| **Physique/Champs** | Rayon Champ | $R_{\text{diff}}$ | $2$ | Rayon d'influence des champs $E$ et $C$ dans le vide. |
| | Décroissance Champ | $\alpha_{\text{field}}$ | $1.0$ | Pente de l'atténuation exponentielle des champs. |
| **Neural/Hebb** | Ticks Neuraux/Phys | $N$ | $5$ | Nombre d'itérations de la Loi Neurale par tick physique. |
| | Coût Spike | $C_{\text{spike}}$ | $0.005$ | Énergie soustraite de $E$ à chaque décharge neuronale. |
| | Période Réfractaire | $\text{Ref}_{\text{period}}$ | $2$ | Durée de l'inhibition neuronale (en ticks neuraux). |
| | Decay Synaptique | $\delta_{\text{synapse}}$ | $0.999$ | Facteur d'oubli passif des poids $W$ (Homéostasie). |
...
| Loi | Intention | Formulation Mathématique |
| :--- | :--- | :--- |
| **Loi 1 : Gradient Spatial ($G$)** | Détermine la position relative au barycentre $\Omega$ de l'amas. | $G = \exp(-\lambda_{\text{grad}} \cdot \|\text{Pos}_{\text{soi}} - \Omega\|)$ |
| **Loi 2 : Différenciation ($T$)** | Spécialise irréversiblement les cellules souches ($T=0$). | • Si $G < S_{\text{soma}} \implies T \leftarrow 1$ (Soma/Peau)<br>• Si $G \ge S_{\text{neuro}} \implies T \leftarrow 2$ (Neurone/Cœur) |
...
| Loi | Intention | Formulation Mathématique |
| :--- | :--- | :--- |
| **Loi 3 : Intégration & Spike ($P$)** | Modélise le potentiel d'action et l'intégration synaptique. | $I = \frac{\sum_{v} (P_v \cdot W_v)}{\max(1.0, \sum_{v} W_v)}$<br>$P_{\text{new}} = (P_{\text{old}} \cdot 0.9) + I + \text{Noise}(\text{Seed})$<br>• Si $P_{\text{new}} > S_{\text{fire}}$ :<br>$$P_{\text{new}} \leftarrow 1.0, \quad E_{\text{cost}} \leftarrow E_{\text{cost}} + C_{\text{spike}}, \quad \text{Ref} \leftarrow \text{Ref}_{\text{period}}$$<br>• Registre : $H \leftarrow (H \ll 1) \mid \text{Spike}$ |
| **Loi 4 : Ignition (Broadcast Local)** | Synchronisation régionale pour créer des assemblées neuronales. | Si $P$ est un Spike :<br>$\forall c \text{ tel que } T_c = 2 \text{ et } \text{dist}(c, \text{soi}) \le r_{\text{ignition}} : P_c \leftarrow P_c + 0.1$ |
...
| Loi | Intention | Formulation Mathématique |
| :--- | :--- | :--- |
| **Loi 5 : Plasticité Hebbienne ($W$)** | Apprentissage des corrélations causales via l'historique $H$. | • Si ($P_{\text{soi}}$ actif) ET ($H_{\text{voisin}}$ récent) :<br>$$W_i \leftarrow W_i + \eta_{\text{learn}}$$<br>• Sinon :<br>$$W_i \leftarrow W_i - (0.1 \cdot \eta_{\text{learn}})$$<br>• Homéostasie :<br>$$W_i \leftarrow \min(1.0, \max(0.0, W_i \cdot \delta_{\text{synapse}}))$$ |
| **Loi 6 : Mémoire Métabolique ($M$)** | Conserve la trace des ressources passées avec oubli progressif. | $M \leftarrow \max(M \cdot 0.99, \; \max_{v \in \text{Voisins}}(E_v))$ |
...
| Loi | Intention | Formulation Mathématique |
| :--- | :--- | :--- |
| **Loi 7 : Métabolisme et Mort** | Coût de l'existence et conditions de survie. | $D \leftarrow D + \Delta D_{\text{tick}}$<br>$E \leftarrow E - k_{\text{thermo}} - E_{\text{cost}}$<br>$L \leftarrow L + \Delta L_{\text{tick}}$<br>• Si $E \le 0$ OU $C > S_c \implies \text{Mort}$ |
| **Loi 8 : Champs d'Influence** | Diffusion de l'influence de la matière dans le vide. | $\text{Val}(v) = \sum_{c} (\text{Signal}_c \cdot \exp(-\alpha_{\text{field}} \cdot \text{dist}(v, c)))$<br>$\text{BonusChamp}(v) = (k_E \cdot \text{Map}_E(v)) - (k_C \cdot \text{Map}_C(v))$ |
| **Loi 9 : Mouvement avec Adhésion** | Déplacement résultant des forces internes et externes. | $\text{Score}(v) = k_D D - k_C C + k_M \frac{M}{A+1} + \text{BonusChamp}(v) + \text{BonusAdh}(v) - C_{\text{mvt}}$<br>$\text{BonusAdh}(v) = K_{\text{adh}} \cdot N_{\text{voisins}}(v)$ |
...
| Loi | Intention | Formulation Mathématique |
| :--- | :--- | :--- |
| **Loi 10 : Osmose (Échange d'Énergie)** | Équilibrage passif et conservateur entre cellules compatibles. | $\Delta = (E_a - E_b) \cdot F_{\text{ech}}$<br>$\Delta_{\text{safe}} = \min(\Phi_{\max}, \max(-\Phi_{\max}, \Delta))$<br>$E_a \leftarrow E_a - \Delta_{\text{safe}}, \quad E_b \leftarrow E_b + \Delta_{\text{safe}}$<br>• Condition : $|R_a - R_b| < \text{Tol}_R$<br>• Unicité : traiter la paire si $\text{ID}_a < \text{ID}_b$ |
| **Loi 11 : Reproduction (Mitose)** | Division cellulaire avec conservation stricte de l'énergie. | $E_{\text{fille}} = E_{\text{mère}} \leftarrow \frac{E_{\text{mère}}}{2}$<br>Mutation : $R_{\text{fille}} = \min(1.0, \max(0.0, R_{\text{mère}} + \delta_{\text{mut}}))$ |
| **Interaction Forte (Contact Social)** | Échange direct de stress et d'ennui par contact. | $C \leftarrow C + 0.1 \cdot C_{\text{voisin}}$ (Friction sociale)<br>$L \leftarrow L - 0.1 \cdot L_{\text{voisin}}$ (Satisfaction sociale) |
...
2. **BOUCLE NEURALE (Rapide) :** Répéter $N$ fois les Lois 3 et 4 (Intégration $P$ et Ignition). Accumuler $E_{\text{cost}}$.
...
4. **RÉSOLUTION (Écriture Grille $t+1$) :**
   ...
   * **Mouvements :** Résoudre les conflits (Loi 9) : priorité à **$D$ décroissant**. Appliquer le coût réel : $E \leftarrow E - C_{\text{mvt}}$ pour le gagnant.
   * **Reproduction :** Résoudre les conflits (Loi 11) : priorité à **$E$ décroissant**. Appliquer la conservation $E_{\text{mère}} / 2$.
   * **Interaction Forte :** Appliquer l'échange $C$ et $L$.
5. **FINALISATION :** Appliquer le Clamping sur $C, R, S_c, W$. Permuter `Grid_Read` $\leftrightarrow$ `Grid_Write`.
...
| | Stress | $C$ | Dommage structurel ou pression interne, borné dans $[0,1]$. |
| | Seuil Crit. | $S_c$ | Résistance génétique au Stress. Si $C > S_c$, la cellule meurt. |
...
| **Neural** | Potentiel | $P$ | Charge électrique de la cellule neuronale, bornée dans $[-1, 1]$. |
| | Réfractaire | $\text{Ref}$ | Compteur de ticks de repos de la cellule neuronale après une activation (*spike*). |
| | Coût Pend. | $E_{\text{cost}}$ | Accumulateur de coût énergétique lié à l'activité neurale, facturé à chaque cycle physique. |
...
* **Mouvement et Interaction :** Le mouvement n'est jamais aléatoire ; c'est une décision calculée. Une cellule évalue les cases vides voisines en leur attribuant un score d'attractivité selon une formule mathématique :
$$
\text{Score}(v) = k_D D - k_C C + k_M \frac{M}{A+1} + \text{BonusChamp}(v) + \text{BonusAdh}(v) - C_{\text{mvt}}
$$
Cette formule intègre les pulsions internes (faim $D$, stress $C$), les influences externes des Champs (attraction vers des zones riches en énergie, répulsion des zones stressantes), la cohésion des tissus (adhésion avec les cellules de même type) et un coût énergétique explicite ($C_{\text{mvt}}$). En cas de conflit, la cellule ayant la plus grande valeur de $D$ l'emporte.
* **Métabolisme et Mort :** La survie est une lutte contre l'entropie. À chaque cycle, une cellule consomme une quantité d'énergie de base ($k_{\text{thermo}}$) pour exister, à laquelle s'ajoute le coût de son activité neurale ($E_{\text{cost}}$). Simultanément, sa "dette de besoin" ($D$) augmente, matérialisant une faim croissante. La mort est inévitable et survient sous deux conditions : l'inanition ($E \le 0$) ou une surcharge de stress ($C > S_c$).
* **Reproduction et Stabilité :** La croissance se fait par mitose. Une cellule accumulant suffisamment d'énergie peut se diviser. Ce processus obéit à deux principes stricts. Premièrement, la conservation stricte de l'énergie : l'énergie de la mère est exactement divisée en deux pour former la fille ($E_{\text{fille}} = E_{\text{mère}} / 2$). Deuxièmement, la mutation déterministe : les gènes ($R, S_c$) de la fille subissent une variation calculée par un algorithme de hachage basé sur ses coordonnées, assurant une évolution traçable et reproductible.
* **Osmose et Cohésion :** Le mécanisme fondamental permettant la multicellulairité est l'échange d'énergie passif, ou osmose. Une cellule peut transférer une partie de son énergie à un voisin si leur similarité génétique ($R$) est suffisante ($|R_a - R_b| < \text{Tol}_R$). Ce partage de ressources, borné pour garantir la stabilité, permet de créer des "tissus" où les cellules s'entraident, formant des organismes stables capables de survivre bien plus longtemps que des individus isolés.
...
* **Morphogenèse et Spécialisation :** Un organisme n'est pas une masse informe de cellules identiques. Une cellule souche calcule sa position relative au sein de l'organisme via une variable Gradient spatial ($G$). En fonction de cette position — centrale ou périphérique — elle se différencie de manière irréversible en Neurone ($T=2$, formant le "cerveau") ou en Soma ($T=1$, formant la "peau" protectrice). Ce processus permet la formation spontanée d'une anatomie de base.
* **Dynamique Neurale :** Les neurones opèrent sur une échelle de temps rapide, traitant l'information via un réseau de *spikes*. Chaque neurone possède un potentiel électrique ($P$). Il intègre les signaux de ses voisins, pondérés par des poids synaptiques. Si ce potentiel dépasse un seuil d'activation ($S_{\text{fire}}$), le neurone "tire" (*spike*), propage un signal, accumule un coût énergétique ($C_{\text{spike}}$) et entre dans une courte période réfractaire ($\text{Ref}$) où il est inhibé. L'historique des spikes est conservé dans une mémoire à court terme (Historique $H$).
* **Apprentissage et Plasticité :** Le réseau n'est pas statique ; il apprend. Le SED implémente une règle d'apprentissage Hebbien : "les neurones qui s'activent ensemble se lient ensemble". Concrètement, si un neurone décharge, il renforce ses connexions synaptiques ($W$) vers les voisins dont l'historique d'activité ($H$) révèle une activation dans les trois derniers cycles temporels, créant ainsi des associations causales. Pour garantir la stabilité du réseau, une règle d'homéostasie ($\delta_{\text{synapse}}$) dégrade lentement tous les poids, forçant le système à ne conserver que les connexions pertinentes.
