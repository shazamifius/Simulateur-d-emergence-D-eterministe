    ...
    for (const auto& cell : grid) {
        if (cell.is_alive) {
            ...
            auto hash_combine = [&](float v) {
                ...
                cell_hash ^= hasher(v) + 0x9e3779b9 + (cell_hash << 6) + (cell_hash >> 2);
            };
            hash_combine(cell.E);
            hash_combine(cell.C);
            hash_combine(cell.R);
...
int main() {
    ...
    size_t hash1 = compute_grid_hash(monde1.getGrille());
    ...
    size_t hash2 = compute_grid_hash(monde2.getGrille());
    ...
    if (hash1 == hash2) {
        ...
        std::cerr << "FAILURE: Hashes do not match. Determinism broken." << std::endl;
        return 1;
    }
}
