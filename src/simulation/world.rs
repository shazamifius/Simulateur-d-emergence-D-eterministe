...
use super::laws::{
    appliquer_loi_metabolisme, deterministic_mutation, euclidean_distance, evaluer_division,
    evaluer_mouvements, evaluer_osmose, evaluer_psychisme,
};
...
where
    S: serde::Serializer,
{
    let v: Vec<(&ChunkKey, &Chunk)> = chunks.iter().collect();
    v.serialize(serializer)
}

pub fn deserialize_chunks<'de, D>(
    deserializer: D,
) -> Result<HashMap<ChunkKey, Chunk>, D::Error>
where
    D: serde::Deserializer<'de>,
{
    let v: Vec<(ChunkKey, Chunk)> = Deserialize::deserialize(deserializer)?;
...
impl WorldMap {
    ...
    pub fn get_cell_global_mut(&mut self, wx: i32, wy: i32, wz: i32) -> Option<&mut Cell> {
        ...
        let (cx, cy, cz, lx, ly, lz) = Self::world_to_chunk_coords(wx, wy, wz);
        self.get_chunk_mut(cx, cy, cz)
            .map(|chk| chk.get_cell_mut(lx, ly, lz))
    }
...
impl MondeSED {
    ...
    pub fn sync_read_map(&mut self) {
        ...
        if self.read_map.chunks.len() > self.world_map.chunks.len() {
            ...
            self.read_map
                .chunks
                .retain(|key, _| world_chunks.contains_key(key));
        }
    }
                                ...
                                if cell.is_alive && cell.t == CellType::Neurone {
                                    if cell.r#ref > 0 {
                                        ...
                                        for &(dx, dy, dz, w_idx) in &neighbors_coords {
                                            ...
                                            let neighbor_p = read_map
                                                .get_cell_global(nx, ny, nz)
    ...
    pub fn avancer_temps(&mut self) {
        ...
        let mut chunk_intents: Vec<ChunkIntentions> = self
            .world_map
            .chunks
            .par_iter_mut()
            .map(|(&key, chunk)| {
                ...
                for lx in 0..CHUNK_SIZE {
                    for ly in 0..CHUNK_SIZE {
                        for lz in 0..CHUNK_SIZE {
                            ...
                            for dz in -1..=1 {
                                for dy in -1..=1 {
                                    for dx in -1..=1 {
                                        ...
                                        if let Some(v) =
                                            read_map.get_cell_global(wx + dx, wy + dy, wz + dz)
                                            && v.is_alive
                                            && v.e > max_e
                                        {
                                            max_e = v.e;
                            ...
                            if let Some(div) =
                                evaluer_division(wx, wy, wz, cell, read_map, &self.params)
                            {
        ...
        chunk_intents.sort_by(|a, b| {
            if a.chunk_key.x != b.chunk_key.x {
                a.chunk_key.x.cmp(&b.chunk_key.x)
            } else if a.chunk_key.y != b.chunk_key.y {
                a.chunk_key.y.cmp(&b.chunk_key.y)
            } else {
                a.chunk_key.z.cmp(&b.chunk_key.z)
            }
        });
        ...
        for chunk_i in &chunk_intents {
            for e in &chunk_i.echanges_energie {
                let src_alive = self
                    .world_map
                    .get_cell_global(e.source.0, e.source.1, e.source.2)
                    .map(|c| c.is_alive && c.t != CellType::Static)
                    .unwrap_or(false);
                let dst_alive = self
                    .world_map
                    .get_cell_global(e.destination.0, e.destination.1, e.destination.2)
                    .map(|c| c.is_alive && c.t != CellType::Static)
                    .unwrap_or(false);

                if src_alive && dst_alive {
                    if let Some(src_cell) = self
                        .world_map
                        .get_cell_global_mut(e.source.0, e.source.1, e.source.2)
                    {
                        src_cell.e -= e.montant_energie;
                    }
                    if let Some(dst_cell) = self.world_map.get_cell_global_mut(
                        e.destination.0,
                        e.destination.1,
                        e.destination.2,
                    ) {
        ...
        for (dest, src) in dest_occupees {
            let (cx_d, cy_d, cz_d, lx_d, ly_d, lz_d) =
                WorldMap::world_to_chunk_coords(dest.0, dest.1, dest.2);
            let (cx_s, cy_s, cz_s, lx_s, ly_s, lz_s) =
                WorldMap::world_to_chunk_coords(src.0, src.1, src.2);

            let cell_src = self
                .world_map
                .get_chunk(cx_s, cy_s, cz_s)
                .map(|chk| *chk.get_cell(lx_s, ly_s, lz_s));
        ...
        toutes_divisions.par_sort_unstable_by(|a, b| {
            if a.destination_fille != b.destination_fille {
                a.destination_fille.cmp(&b.destination_fille)
            } else if (a.energie_mere - b.energie_mere).abs() > 0.0001 {
                b.energie_mere
                    .partial_cmp(&a.energie_mere)
                    .unwrap_or(std::cmp::Ordering::Equal)
            } else {
        ...
        for (dest, src) in div_dest_occupees {
            ...
            if let Some(cell_mere) = self.world_map.get_cell_global_mut(src.0, src.1, src.2)
                && cell_mere.is_alive
                && cell_mere.e > self.params.seuil_energie_division
            {
        ...
        for chunk_i in &chunk_intents {
            for psy in &chunk_i.echanges_psychiques {
                if let Some(src_cell) =
                    self.world_map
                        .get_cell_global_mut(psy.source.0, psy.source.1, psy.source.2)
                    && src_cell.is_alive
                {
