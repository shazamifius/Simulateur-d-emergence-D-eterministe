...
static int selected_cell_coords[3] = {
    -1, -1, -1}; // Coordonnées de la cellule sélectionnée
...
int main() {
  ...
  // rlImGuiSetup MUST be called before accessing any ImGui functions (like
  // GetStyle). It creates the context and initializes the backend.

  rlImGuiSetup(true);
  std::cout << "[DEBUG] rlImGui initialized." << std::endl;

  // --- Custom Theme ---
  // Now that the context is created by rlImGui, we can safely apply the style.
  ...
  std::cout << "[DEBUG] Style Configured." << std::endl;
  ...
  while (!WindowShouldClose()) {
    // std::cout << "[DEBUG] Frame Start." << std::endl; // Commented to avoid
    // spam, uncomment if crash happens inside loop
    ...
    if (!ImGui::GetIO().WantCaptureMouse) {
      ...
      if (wheel != 0) {
        ...
        if (distance < 2.0f)
          distance = 2.0f; // Empêche de traverser la cible
        if (distance > 100.0f)
          distance = 100.0f; // Distance maximale
        camera.position = Vector3Add(
            camera.target,
            Vector3Scale(Vector3Normalize(Vector3Negate(toTarget)), distance));
      }
      ...
      if (IsMouseButtonDown(MOUSE_BUTTON_MIDDLE) && IsKeyDown(KEY_LEFT_SHIFT)) {
        ...
        Vector3 panVector = Vector3Add(Vector3Scale(right, -delta.x * panSpeed),
  ...
  std::cout << "[DEBUG] Close Window." << std::endl;
...
void ResetSimulation() {
  // Si l'utilisateur a choisi d'utiliser une graine aléatoire, en générer une
  // nouvelle.
  if (use_random_seed) {
...
void DrawUI() {
  ImGui::Begin("Panneau de Contrôle", nullptr,
  ...
  if (ImGui::BeginTabBar("MainTabBar")) {
    ...
    if (ImGui::BeginTabItem("Contrôle")) {
      ...
      if (monde) {
        if (simulation_running) {
          ...
          float button_width = ImGui::GetContentRegionAvail().x * 0.5f -
                               ImGui::GetStyle().ItemSpacing.x * 0.5f;
          if (ImGui::Button("Démarrer", ImVec2(button_width, 0)))
      ...
      if (monde) {
        ...
        ImGui::Text("Cellules vivantes: %d",
                    monde->getNombreCellulesVivantes());
        if (!cell_count_history.empty()) {
          ImGui::Text("Historique du nombre de cellules :");
          ImGui::PlotLines("##cell_history", cell_count_history.data(),
                           static_cast<int>(cell_count_history.size()), 0,
    ...
    if (monde && ImGui::BeginTabItem("Paramètres")) {
      ...
      ImGui::SeparatorText("Loi 0: Thermodynamique");
      ImGui::SliderFloat("K_THERMO (Coût Vie)", &params.K_THERMO, 0.0f, 0.01f,
                         "%.4f");
      ImGui::SliderFloat("D_PER_TICK (Faim)", &params.D_PER_TICK, 0.0f, 0.01f,
                         "%.4f");
      ImGui::SliderFloat("L_PER_TICK (Ennui)", &params.L_PER_TICK, 0.0f, 0.01f,
                         "%.4f");
      ImGui::SliderFloat("Taux Oubli (M)", &params.TAUX_OUBLI, 0.0f, 0.1f);
      ...
      ImGui::SliderFloat("Seuil Énergie", &params.SEUIL_ENERGIE_DIVISION, 0.1f,
                         5.0f);
      ImGui::SliderFloat("Coût Division", &params.COUT_DIVISION, 0.0f, 1.0f);

      ImGui::SeparatorText("Loi 3: Champs");
      ImGui::SliderFloat("Rayon Diffusion", &params.RAYON_DIFFUSION, 1.0f,
                         5.0f);
      ImGui::SliderFloat("Alpha (Atténuation)", &params.ALPHA_ATTENUATION, 0.1f,
                         5.0f);

      ImGui::SeparatorText("Loi 4: Osmose");
      ImGui::SliderFloat("Facteur Échange", &params.FACTEUR_ECHANGE_ENERGIE,
                         0.0f, 0.5f, "%.3f");
      ImGui::SliderFloat("Seuil Diff.", &params.SEUIL_DIFFERENCE_ENERGIE, 0.0f,
                         1.0f);
      ImGui::SliderFloat("Max Flux", &params.MAX_FLUX_ENERGIE, 0.0f, 0.2f);
      ImGui::SliderFloat("Tolérance Génétique", &params.SEUIL_SIMILARITE_R,
                         0.0f, 1.0f);
  ...
  ImGui::SetNextWindowPos(ImVec2(10, GetScreenHeight() - 110),
                          ImGuiCond_Always);
  ImGui::SetNextWindowSize(ImVec2(250, 100), ImGuiCond_Always);
  ImGui::Begin("Légende", nullptr,
  ...
  if (cell_is_selected) {
    ...
    if (monde) {
      ...
      const Cellule cell =
          monde->getGrille()[x + y * monde->getTailleX() +
                             z * monde->getTailleX() * monde->getTailleY()];
...
void Draw3DVisualization(Camera &camera) {
  ...
  for (int i = 0; i < grille.size(); ++i) {
    ...
    if (cellule.is_alive) {
      ...
      if (cell_is_selected && selected_cell_coords[0] == x &&
          selected_cell_coords[1] == y && selected_cell_coords[2] == z) {
        DrawSphereWires(position, radius + 0.05f, 8, 8,
